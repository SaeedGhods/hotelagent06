...
async def incoming_call(request: Request):
    ...
    try:
        ...
        # IMPORTANT: When running on Render with Twilio, relative URLs for <Play> often fail 
        # because Twilio doesn't know the base domain.
        # We MUST provide the absolute URL.
        # Check if PUBLIC_URL is set, otherwise default to the known Render URL.
        public_url = os.getenv("PUBLIC_URL", "https://hotel-agent-lhvi.onrender.com")
        ...
        # Use absolute URL for the audio file
...
async def handle_speech(request: Request, SpeechResult: str = Form(None)):
    ...
    try:
        ...
        if SpeechResult:
            ...
            gather.play(f"{public_url}/speak?text={encoded_ai_text}")
